...
<?php
	...
	if(isset($_POST['prod'])){
		...
		$Qry1 = mysqli_query($conn,"SELECT * FROM products WHERE Code='$prod'");
	...
	<div class="cont">
					...
					while($row=mysqli_fetch_assoc($Qry)){
						...
						<form method="post">
							<tr <?php if($code==$row['Code']){?>bgcolor="#f2d9e6"<?php } ?>>
								...
								<td><input type="checkbox" value="<?php echo $row['Code']; ?>" name="prod" onclick="submit()" 
		...
		<div class="x">
			...
			<div>
				<table class="table">
					<tr>
						<th colspan=4>Sales for <?php echo $code."--".$name; ?> For the Period <?php echo $d1;?> To <?php echo $d2;?></th>
					</tr>
					<tr>
						<th>Date</th>
						<th>Quantity</th>
					...
					<?php 
						$total =0;
						$Qry = mysqli_query($conn,"SELECT Sdate,Quantity,Price,(Quantity*Price) AS Total FROM sales 
						WHERE Code ='$codex' AND Sdate>='$d1' AND Sdate<='$d2' ORDER BY Sdate ");
						if(!$Qry){
