...
<?php
	...
	if(isset($_POST['emp'])){
		...
		$Qry1 = mysqli_query($conn,"SELECT * FROM operators WHERE Num='$employee'");
	...
	<div class="cont">
		<div class="noprint">
			<table class="table">
				...
				<?php
					$Qry = mysqli_query($conn,"SELECT * FROM operators ");
					while($row=mysqli_fetch_assoc($Qry)){
						?>
						<form method="post">
							<tr <?php if($code==$row['Num']){?>bgcolor="#f2d9e6"<?php } ?>>
								<td><?php echo $row['Num']; ?></td>
								<td><?php echo $row['Name']; ?></td>
								<td><input type="checkbox" value="<?php echo $row['Num']; ?>" name="emp" onclick="submit()" 
								<?php if($code==$row['Num']){?>checked<?php } ?>></td>
							</tr>
						</form>
						<?php
					}
		...
		<div class="x">
			...
			<table class="table">
				<tr>
					<th colspan=5>Sales by <?php echo $code."--".$name; ?> For the Period <?php echo $d1;?> To <?php echo $d2;?></th>
				</tr>
				<tr>
					<th>Date</th>
					<th>Item</th>
				...
				<?php 
					...
					WHERE Employee ='$codex' AND Sdate>='$d1' AND Sdate<='$d2' ORDER BY Sdate ");
					if(!$Qry){
